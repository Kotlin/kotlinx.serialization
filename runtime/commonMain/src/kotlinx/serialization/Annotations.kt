 ...
 * Applying [Serializable] to the Kotlin class instructs the serialization plugin to automatically generate [KSerializer]
 * for the current class, that can be used to serialize and deserialize the class.
 * The generated serializer can be accessed with `.serializer()` extension method that is generated by the plugin as well.
 * ```
 * @Serializable
 ...
 * class RgbPixel(val red: Byte, val green: Byte, val blue: Byte)
 *
 * @Serializable
 * class RgbExample(
 ...
 *     p3: RgpPixel // Serialize as 3 byte components, e.g. { "red": 255, "green": 255, "blue": 0 }
 * )
 * ```
 * In this example, each pixel will be serialized using different data representation.
 *
 * For classes with generic type parameters, `serializer()` method requires one additional argument per each generic type parameter:
 * ```
 * @Serializable
 * class Box<T>(value: T)
 *
 ...
 *
 * In order to generate `serializer` method that is not a method on the particular instance, the class should have a companion object, either named or unnamed.
 ...
 * If `serializer` method already exists, it will be matched against the desired signature `serializer(vararg type params): KSerializer<T>`.
 * On successful match, the existing method will be used as [KSerializer] source instead of auto-generated one.
 * On unsuccessful match, a compiler error occurs.
 *
 * @see UseSerializers
 * @see Serializer
 */
 ...
 *
 * @Serializable
 * class Foo(val myInt: Int)
 *
 * @Serializable
 * @SerialName("Bar")
 * class Bar(@SerialName("int") val myInt: Int)
 *
 * fun printNames(descriptor: SerialDescriptor) {
 *     println(descriptor.serialName)
 *     println(descriptor.getElementName(0))
 * }
 *
 * // Prints "foo.Foo" and "myInt"
 * printNames(Foo.serializer().descriptor)
 *
 * // Prints "Bar" and "int"
 * printNames(Bar.serializer().descriptor)
 * ```
 * Element names and serial names are used by text-based serial formats in order to encode the name of the class or
 ...
 * ```
 * /**
 *  * Specifies ProtoBuf unique id for a field in the protobuf message, assigned to a Kotlin property
 *  */
 * @SerialInfo
 * @Target(AnnotationTarget.PROPERTY)
