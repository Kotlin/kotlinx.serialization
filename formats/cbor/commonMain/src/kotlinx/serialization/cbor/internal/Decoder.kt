...
internal open class CborReader(override val cbor: Cbor, internal val parser: CborParser) : AbstractDecoder(),
    ...
    private fun nextNumberWithinRange(
        from: Long,
        to: Long,
    ...
    override fun decodeByte() = nextNumberWithinRange(
        Byte.MIN_VALUE.toLong(), Byte.MAX_VALUE.toLong(), UByte.MAX_VALUE.toLong(), "Byte"
    ).toByte()

    override fun decodeShort() = nextNumberWithinRange(
        Short.MIN_VALUE.toLong(), Short.MAX_VALUE.toLong(), UShort.MAX_VALUE.toLong(), "Short"
    ).toShort()

    override fun decodeChar() = nextNumberWithinRange(
        Char.MIN_VALUE.code.toLong(), Char.MAX_VALUE.code.toLong(),
        /* no unsigned type for Char */ -1, "Char"
    ).toInt().toChar()

    override fun decodeInt() = nextNumberWithinRange(
        Int.MIN_VALUE.toLong(), Int.MAX_VALUE.toLong(), UInt.MAX_VALUE.toLong(), "Int"
    ).toInt()
