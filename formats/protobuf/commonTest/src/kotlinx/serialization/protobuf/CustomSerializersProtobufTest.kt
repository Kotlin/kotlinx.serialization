...
class CustomSerializersProtobufTest {
    ...
    fun writeCustom() {
        val a = A(B(2))
        val j = protoBufWithB()
        val s = j.encodeToHexString(a).toUpperCase()
        assertEquals("0802", s)
    }
    ...
    fun writeCustomList() {
        ...
        val j = protoBufWithB()
        val s = j.encodeToHexString(obj).toUpperCase()
    ...
    fun readCustomInvertedOrder() {
        val obj = C(1, 2)
        val j = ProtoBuf()
        val s = j.decodeFromHexString<C>("10020801")
        assertEquals(obj, s)
    }
    ...
    fun readCustomOmitDefault() {
        val obj = C(b = 2)
        val j = ProtoBuf()
        val s = j.decodeFromHexString<C>("1002")
        assertEquals(obj, s)
    }
